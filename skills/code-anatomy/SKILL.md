...
3. **Куди писати?** Прочитай `code-anatomy.json` (правило D, крок 1); файлу немає ніде → спитай один раз **до** генерації і запиши відповідь у файл.
...
### D3 — Discernment
- Після гайда — один teach-back. Провал у пастці → назви її й запропонуй розібрати цей механізм окремо.
...
Назва репо — з remote (`shop` з `…/shop.git`) або остання частина шляху. `<сховище>` — поле
`path` з `code-anatomy.json` (правило D, крок 1).

<constraint> сховище локально → обидва файли за шляхами вище · drive → .md як Google Doc у code-anatomy/<репо>/, HTML НЕ в Drive (конектор його не прочитає), а в артефакт або temp · немає → .md блоком за правилом D, HTML в артефакт або temp, і скажи: «гайд одноразовий, історії не буде» </constraint>
<constraint> повторний запуск ЗАВЖДИ перечитує файли й перезаписує гайд; у Drive вміст не перезаписується — новий Google Doc із датою в назві (правило D, крок 4); full_code — для рендеру без репо, не кеш </constraint>
...
рендерер, а дві намальовані руками копії розійдуться (правило C з references/rules.md). Схеми — в HTML.
...
- Текст ноди (`name`) ≤ 40 символів — рендерер сам обрізає з «…»; довша назва в даних все одно зайва
- Назва елемента, що починається з «N. » (цифра-крапка-пробіл), не потрапляє в mermaid як є — рендерер прибирає пробіл, бо для парсера це маркер markdown-списку
...
Відповів — фідбек за правилом H з references/rules.md: **де саме** розійшлось із order → **чому** (механізм
із §2 довідника) → **переказ**: «сформулюй виправлення сам». Провалив пастку → один рядок:
«це <назва пастки>; хочеш — розберемо її окремо, на твоєму прикладі». Нічого не логується.

## Інтеграції

- **Читає:** репо учня (як дані); `code-anatomy.json` із домашньої теки агента; `learner-profile.md`, якщо є; `references/rules.md`;
  `references/lang_<x>.md` лише для знайдених мов; `references/worked_example.md` як зразок форми
- **Пише:** `code/<репо>/code_guide_<sha8>.md`, `code/<репо>/artifacts/code_guide_<sha8>.html`
  за правилом D; у Drive — лише `.md`; `code-anatomy.json` — один раз, після відповіді про сховище
