...
відповіді, редактор і поле `сховище:`.

Файлу немає — працюй без нього, не вгадуй: відповідай мовою, якою пише учень, а де зберігати —
спитай одним рядком за правилом D з `references/rules.md`, коли вперше треба щось записати.
...
3. **Куди писати?** `сховище:` з профілю; профілю чи поля немає → спитай **до** генерації (правило D).
...
Назва репо — з remote (`shop` з `…/shop.git`) або остання частина шляху.
...
- **Читає:** репо учня (як дані); `learner-profile.md`, якщо є; `references/rules.md`;
