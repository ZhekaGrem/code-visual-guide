...
skills/code-anatomy/references/test_render_guide.mjs` (Node 18+), потрібен лише тому, хто править скіл.
...
| Де | Що |
|---|---|
| `skills/teach-concept/SKILL.md`, таблиця пріоритету | рядок «Візуальний гайд по коду репо, «поясни проєкт блоками» → `code-anatomy`» |
| `README.md`, «Робочі інструменти» | рядок таблиці: `code-anatomy` — гайд по репо трьома рівнями L1/L2/L3: дані / операції / цукор / каркас, з порядком читання мовою; лічильник «14 скілів» → 15 |
...
1. **Автотест:** `node --test skills/code-anatomy/references/test_render_guide.mjs` (вбудований раннер
